...
> 底线：**LLM 只生成文本、只做建议，不参与任何数值决策，全程 HITL 人工确认**（ADR-0001）。数值全部来自大数据基准 + 本系统规则，与 LLM 解耦——LLM 失败时数值链路不受影响，仅文本降级为规则模板/留空待人工。
...
|-----------|---------|--------------|------|------|------|
| `forecastBasisAgent` | 预测**业务语言依据**文本（中/英）：解释影响预测的关键因子 | 品牌/类型/周期/基准值/三档/历史统计（对象以 ID 代名，无 PII） | `basisText` | `history_stat`（只读，经 HistoryDataQueryFacade） | 结果随预测展示，人工确认总目标时把关 |
| `reviewAgent` | **复盘优化建议**文本：基于达成/偏差生成业务语言建议 | 各层级最终达成率/偏差/超阈对象（ID 代名） | 建议摘要文本 | `review_stat`（只读，经 tracking 内部达成汇总） | 生成后**人工审阅**再随报告展示（OQ-5 边界） |
