...
| split_levels | JSON | 拆解层级列表,如 `["REGION","DEALER","MODEL"]`(对齐 R6 新口径:区域→目标对象→机型,最多 3 层;对象层取值为 DEALER/SHOP/PROMOTER 之一,按目标创建时选定的对象维度;枚举值统一用 SHOP,对齐基线 ObjectLevel) |
| weight_historical_rate | INT | 历史达成率权重占比 0-100 |
| weight_capital | INT | 资金盘权重占比 0-100 |
| weight_territory | INT | 阵地数权重占比 0-100 |
| weight_avg_sales_3m | INT | 过去3月平均销量权重占比 0-100;四项之和=100(入库校验) |
...
1. **权重占比归一(源级)**:取 `weight_historical_rate/capital/territory/avg_sales_3m`。若某源数据不可用(AC-07),该占比归零,剩余源重新归一化到 100%。全源不可用则退化为平均分摊,记录降级。
2. **对象指标归一(对象级)**:对每个可用源 k,取各兄弟对象指标 `m_k`(历史达成率、过去3月平均销量经 `HistoryDataQueryFacade`;资金盘/阵地数见 OQ-02/03 占位),兄弟间归一化使 `Σ m_k = 1`。
...
请求/响应 DTO:`SplitRuleConfigDTO`(country_code/brand_code/target_type/**rule_tier**/split_levels/weight_historical_rate/weight_capital/weight_territory/weight_avg_sales_3m/historical_periods/balance_mode/config_version/enabled),置 **common** 包(契约唯一来源)。错误统一走 `GoalBizException(GoalErrorCode.VALIDATION_FAILED)`。

### 入库校验(强校验,对齐 AC-08b)

保存(POST/PUT)时,`weight_historical_rate + weight_capital + weight_territory + weight_avg_sales_3m = 100`,否则拒绝保存并返 `VALIDATION_FAILED`,提示"四项权重之和必须为 100%"。四项均须 0-100 整数。balance_mode 须为枚举值之一。
