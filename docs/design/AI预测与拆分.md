...
|------|------|------|
| id | BIGINT PK | |
| country_code / brand_code / target_type | VARCHAR | 适用范围;唯一键 `uk_scope(country_code,brand_code,target_type,enabled)` |
...
|------|------|------|
| id | BIGINT PK | |
| country_code / brand_code / target_type | VARCHAR | 适用范围;唯一键 `uk_scope` |
...
- `PredictionRequest`:targetPlanId, countryCode, brand, targetType, targetPeriod, objectType(可选机型范围)。
...
| 方法 | 语义 | 错误码触发条件 |
|------|------|--------------|
| `SplitResultDTO split(SplitRequest)` | 接确认总目标,逐层拆分(国家→区域→Dealer→门店→Promoter→机型),写初稿+跑异常,status→DRAFT | `SPLIT_NO_RULE`:该 国家/品牌/目标类型 无 t_split_rule_config(enabled)记录(AC-08) |
...
落地契约基线 §5.2 公式,规则按 `国家×品牌×目标类型` 取 `t_prediction_rule_config`:
...
- 每 call 必传 `RuntimeContext(userId, sessionId="plan-"+targetPlanId)` + `GoalAgentContext`;工具 `history_stat` 经 `HistoryDataQueryFacade` 只读取数,Reactor 线程内重建 ThreadLocal、`finally` 清理。
...
| 新建配置 | `POST /goal/config/split-rule` | 唯一键 `uk_scope`(国家+品牌+目标类型)已存在则拒绝,改用更新 | `goal:split-rule:manage` |
...
请求/响应 DTO:`SplitRuleConfigDTO`(country_code/brand_code/target_type/split_levels/weight_historical_rate/weight_capital/weight_territory/historical_periods/balance_mode/config_version/enabled),置 **common** 包(契约唯一来源)。错误统一走 `GoalBizException(GoalErrorCode.VALIDATION_FAILED)`。
